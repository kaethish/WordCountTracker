<script>
  ...
  // This flag prevents the reactive statement from running until after data is loaded.
  let loaded = false;
  ...
  onMount(() => {
    ...
    if (browser) {
      const storedData = localStorage.getItem('wordCountAppData');
      if (storedData) {
        const parsed = JSON.parse(storedData);
        wordGoal = parsed.wordGoal || 0;
        goalMode = parsed.goalMode || 'input';
        baselineCount = parsed.baselineCount || 0;
        currentProgress = parsed.currentProgress || 0;
        logs = parsed.logs || [];
      }
      // Now that data is loaded, allow reactive persistence.
      loaded = true;
    }
  });

  // Persist state only after data is loaded.
  $: if (browser && loaded) {
    const data = { wordGoal, goalMode, baselineCount, currentProgress, logs };
    localStorage.setItem('wordCountAppData', JSON.stringify(data));
  }
